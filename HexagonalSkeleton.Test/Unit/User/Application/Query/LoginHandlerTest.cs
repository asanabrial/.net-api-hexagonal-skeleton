...
namespace HexagonalSkeleton.Test.Unit.User.Application.Query
{
    ...
    {        [Theory, AutoData]
        public async Task Login_Should_Return_All_Entities_Without_Deleted_Ones(CancellationTokenSource cts)
        {
            ...
            userReadRepositoryMock.Setup(s => s.GetByEmailAsync(user.Email!, cts.Token)).ReturnsAsync(user);
            authenticationServiceMock.Setup(s => s.GenerateJwtToken(user)).Returns("fake-token");

            var validatorLoginQuery = new LoginQueryValidator();
            var loginQueryHandler = new LoginQueryHandler(
                validatorLoginQuery,
                mediator.Object,
            ...
            // Act
            var resultResponse = await loginQueryHandler.Handle(new LoginQuery(user.Email!, passwordRaw), cts.Token);
        ...
        public async Task Login_Should_Return_NotFound_When_User_Not_Found(CancellationTokenSource cts)
        {
            ...
            userReadRepositoryMock.Setup(s => s.GetByEmailAsync(email, cts.Token)).ReturnsAsync(user);
            ...
            // Act
            var resultResponse =
                await loginQueryHandler.Handle(new LoginQuery(email, password), cts.Token);
        ...
        public async Task Login_Should_Return_Validation_Error_When_Email_Is_Empty(CancellationTokenSource cts)
        {
            ...
            // Act
            var resultResponse = await loginQueryHandler.Handle(new LoginQuery(string.Empty, "password"), cts.Token);
        ...
        public async Task Login_Should_Return_Validation_Error_When_Password_Is_Empty(CancellationTokenSource cts)
        {
            ...
            // Act
            var resultResponse = await loginQueryHandler.Handle(new LoginQuery("email", string.Empty), cts.Token);
            var result = resultResponse as ProblemHttpResult;
            var problemDetails = result!.ProblemDetails as HttpValidationProblemDetails;
